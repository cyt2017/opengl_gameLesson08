...
bool QGLESWIDGET::init_QGW(std::vector<QString> fileName)
{
    ...
    _shader.initialize();
        ...
        {//!启用opengl中的混合，目的是使图像边缘变柔和,
            //!（glBlendFunc）函数，是指定混合函数的算法
            glEnable(GL_BLEND);
            glBlendFunc(GL_SRC_ALPHA,GL_ONE_MINUS_SRC_ALPHA);
        }
        {//!启用深度测试：目的是根据深度值画图.如果不启用深度测试，那么只要是后画的图像就会在前面的视野中
         ...
         *创建顶点缓冲区
         */
            glGenBuffers(1,&_vertexBO);
            /**
         *指定缓冲区类型
         */
            glBindBuffer(GL_ARRAY_BUFFER,_vertexBO);
            /**
         *更新缓冲区数据
         */
            glBufferData(GL_ARRAY_BUFFER,sizeof(vertexs),vertexs,GL_STATIC_DRAW);
            /**
         *更新缓冲区数据,由于opengl是状态机，如果不清零表示使用该数据，
         *那么会影响到后面的数据使用，所以每次用完缓冲区都需要清零操作。
         */
        ...
        {//!索引缓冲区
            /**
         *当使用索引缓冲区后，绘制函数要用（glDrawElements）
         * 由于使用的是以三角形为图元，采用的是一个索引代表一个点，
         * 一个三角形就由三个(点)索引组成，而一个矩形有6个面，1面有2个三角形，那么需要（6x2x3 = 36）个索引
         */
    ...
    {//!帧缓冲区
        /**
          *在没有绘制深度时，可以不启用渲染缓冲区。图像只会根据先后顺序叠加在图像上。
         *1、启用帧（frame）缓冲 2、绑定
         *3、启用渲染（render）缓冲，并绑定
         *4、给渲染赋值
         *5、关闭渲染缓冲
         *6、给帧缓冲区赋值
         *7、关闭帧缓冲区
        ...
        /**
         *(glRenderbufferStorage)函数绘制，渲染buffer的存储内容
         * 参数1、  内容
         * 参数2、  存储的数据类型
         * 参数3,4、需要渲染的范围值
         */
...
void QGLESWIDGET::destroyOpenGLES20()
{
    {//!销毁产生的文理和缓冲区
        ...
        glDeleteTextures(1,&_textureAuto);
        glDeleteBuffers(1,&_vertexBO);
...
void QGLESWIDGET::displayTexMatRot(CELL::matrix4 matView)
{
    ...
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER,_indexBO);
    {
        ...
        glVertexAttribPointer(_shader._position,3,GL_FLOAT,false,sizeof(Vertex),(void *)0);
        glVertexAttribPointer(_shader._uv,2,GL_FLOAT,false,sizeof(Vertex),(void *)12);
        glVertexAttribPointer(_shader._color,4,GL_FLOAT,false,sizeof(Vertex),(void *)20);
#if 0
        glDrawArrays(GL_TRIANGLE_STRIP,0,6);

        {//!调整平移参数后，传给shader，可以在同一个区域中呈现两张相同的图像
            matRot.translate(-1,-1,-2);
